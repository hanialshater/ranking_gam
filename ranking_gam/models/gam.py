...
class GA2M_Paper(nn.Module):
    ...
    def __init__(
        ...
        self.main_towers = nn.ModuleList(
            [PaperTower(1, hidden_dims, dropout) for _ in range(num_features)]
        )
    ...
    def forward(self, x):
        ...
        for i, tower in enumerate(self.main_towers):
            main_scores.append(tower(x_flat[:, i : i + 1]))
        ...
        if self.num_interactions > 0:
            ...
            for i, tower in enumerate(self.interaction_towers):
                ...
                feat_pair = torch.stack([x_flat[:, f1], x_flat[:, f2]], dim=-1)
    ...
    def get_interaction_effect(self, pair_idx, x1_values, x2_values):
        ...
        x_pair = torch.stack([x1, x2], dim=-1).to(next(self.parameters()).device)
        with torch.no_grad():
...
class GAM_Paper(nn.Module):
    ...
    def __init__(self, num_features=136, hidden_dims=None, dropout=0.0):
        super().__init__()
        if hidden_dims is None:
            hidden_dims = [16, 8]
        self.num_features = num_features
    ...
    def forward(self, x):
        ...
        for i, tower in enumerate(self.towers):
            scores.append(tower(x_flat[:, i : i + 1]))
    ...
    def get_main_effect(self, feature_idx, x_values):
        ...
        x_values = x_values.to(next(self.parameters()).device)
        with torch.no_grad():
            return self.towers[feature_idx](x_values).cpu().numpy().flatten()
